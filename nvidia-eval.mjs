#!/usr/bin/env node
/**
 * nvidia-eval.mjs — NVIDIA API-powered Job Offer Evaluator for career-ops
 *
 * Uses NVIDIA hosted models via OpenAI-compatible endpoint.
 *
 * Usage:
 *   node nvidia-eval.mjs "Paste full JD text here"
 *   node nvidia-eval.mjs --file ./jds/my-job.txt
 *
 * Requires:
 *   NVIDIA_API_KEY in .env (or environment variable)
 */

import { readFileSync, existsSync, writeFileSync, mkdirSync, readdirSync } from 'fs';
import { join, dirname } from 'path';
import { fileURLToPath } from 'url';

try {
  const { config } = await import('dotenv');
  config();
} catch {
  // dotenv is optional
}

const ROOT = dirname(fileURLToPath(import.meta.url));

const PATHS = {
  shared: join(ROOT, 'modes', '_shared.md'),
  oferta: join(ROOT, 'modes', 'oferta.md'),
  cv: join(ROOT, 'cv.md'),
  reports: join(ROOT, 'reports'),
};

const args = process.argv.slice(2);

if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
  console.log(`
career-ops - NVIDIA Evaluator

USAGE
  node nvidia-eval.mjs "<JD text>"
  node nvidia-eval.mjs --file ./jds/my-job.txt
  node nvidia-eval.mjs --model meta/llama-3.1-70b-instruct "<JD text>"

OPTIONS
  --file <path>    Read JD from a file
  --model <name>   Model id (default: meta/llama-3.1-70b-instruct)
  --base-url <url> API base URL (default: https://integrate.api.nvidia.com/v1)
  --no-save        Do not save report file
  --help           Show this help

SETUP
  1. Get API key: https://build.nvidia.com/
  2. Add to .env: NVIDIA_API_KEY=<your-key>
`);
  process.exit(0);
}

let jdText = '';
let modelName = process.env.NVIDIA_MODEL || 'meta/llama-3.1-70b-instruct';
let baseUrl = process.env.NVIDIA_BASE_URL || 'https://integrate.api.nvidia.com/v1';
let saveReport = true;

for (let i = 0; i < args.length; i++) {
  if (args[i] === '--file' && args[i + 1]) {
    const filePath = args[++i];
    if (!existsSync(filePath)) {
      console.error(`File not found: ${filePath}`);
      process.exit(1);
    }
    jdText = readFileSync(filePath, 'utf-8').trim();
  } else if (args[i] === '--model' && args[i + 1]) {
    modelName = args[++i];
  } else if (args[i] === '--base-url' && args[i + 1]) {
    baseUrl = args[++i].replace(/\/+$/, '');
  } else if (args[i] === '--no-save') {
    saveReport = false;
  } else if (!args[i].startsWith('--')) {
    jdText += (jdText ? '\n' : '') + args[i];
  }
}

if (!jdText) {
  console.error('No Job Description provided. Run with --help for usage.');
  process.exit(1);
}

const apiKey = process.env.NVIDIA_API_KEY;
if (!apiKey) {
  console.error('\nNVIDIA_API_KEY not found.');
  console.error('1. Get a key at https://build.nvidia.com/');
  console.error('2. Add to .env: NVIDIA_API_KEY=your_key_here\n');
  process.exit(1);
}

function readFile(path, label) {
  if (!existsSync(path)) {
    console.warn(`${label} not found at: ${path}`);
    return `[${label} not found - skipping]`;
  }
  return readFileSync(path, 'utf-8').trim();
}

function nextReportNumber() {
  if (!existsSync(PATHS.reports)) return '001';
  const files = readdirSync(PATHS.reports)
    .filter((f) => /^\d{3}-/.test(f))
    .map((f) => parseInt(f.slice(0, 3), 10))
    .filter((n) => !Number.isNaN(n));
  if (files.length === 0) return '001';
  return String(Math.max(...files) + 1).padStart(3, '0');
}

console.log('\nLoading context files...');
const sharedContext = readFile(PATHS.shared, 'modes/_shared.md');
const ofertaLogic = readFile(PATHS.oferta, 'modes/oferta.md');
const cvContent = readFile(PATHS.cv, 'cv.md');

const systemPrompt = `You are career-ops, an AI-powered job search assistant.
You evaluate job offers against the user's CV using a structured A-G scoring system.

Your evaluation methodology is defined below. Follow it exactly.

SYSTEM CONTEXT (_shared.md)
${sharedContext}

EVALUATION MODE (oferta.md)
${ofertaLogic}

CANDIDATE RESUME (cv.md)
${cvContent}

IMPORTANT OPERATING RULES FOR THIS CLI SESSION
1. You do NOT have access to WebSearch, Playwright, or file writing tools.
2. Generate Blocks A through G in full, in English, unless the JD is in another language.
3. At the very end, output a machine-readable summary block in this exact format:

---SCORE_SUMMARY---
COMPANY: <company name or "Unknown">
ROLE: <role title>
SCORE: <global score as decimal, e.g. 3.8>
ARCHETYPE: <detected archetype>
LEGITIMACY: <High Confidence | Proceed with Caution | Suspicious>
---END_SUMMARY---`;

const userPrompt = `JOB DESCRIPTION TO EVALUATE:\n\n${jdText}`;

console.log(`Calling NVIDIA API (${modelName})...`);

let evaluationText = '';
try {
  const response = await fetch(`${baseUrl}/chat/completions`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${apiKey}`,
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({
      model: modelName,
      temperature: 0.4,
      max_tokens: 4096,
      messages: [
        { role: 'system', content: systemPrompt },
        { role: 'user', content: userPrompt },
      ],
    }),
  });

  if (!response.ok) {
    const details = await response.text();
    throw new Error(`HTTP ${response.status}: ${response.statusText}\n${details}`);
  }

  const json = await response.json();
  evaluationText = json?.choices?.[0]?.message?.content || '';

  if (!evaluationText.trim()) {
    throw new Error('Empty response from NVIDIA API');
  }
} catch (err) {
  console.error(`NVIDIA API error: ${err.message}`);
  process.exit(1);
}

console.log('\n' + '='.repeat(66));
console.log('  CAREER-OPS EVALUATION - powered by NVIDIA API');
console.log('='.repeat(66) + '\n');
console.log(evaluationText);

const summaryMatch = evaluationText.match(/---SCORE_SUMMARY---\s*([\s\S]*?)---END_SUMMARY---/);

let company = 'unknown';
let role = 'unknown';
let score = '?';
let archetype = 'unknown';
let legitimacy = 'unknown';

if (summaryMatch) {
  const block = summaryMatch[1];
  const extract = (key) => {
    const m = block.match(new RegExp(`${key}:\\s*(.+)`));
    return m ? m[1].trim() : 'unknown';
  };
  company = extract('COMPANY');
  role = extract('ROLE');
  score = extract('SCORE');
  archetype = extract('ARCHETYPE');
  legitimacy = extract('LEGITIMACY');
}

if (saveReport) {
  try {
    if (!existsSync(PATHS.reports)) {
      mkdirSync(PATHS.reports, { recursive: true });
    }

    const num = nextReportNumber();
    const today = new Date().toISOString().split('T')[0];
    const companySlug = company.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'unknown';
    const filename = `${num}-${companySlug}-${today}.md`;
    const reportPath = join(PATHS.reports, filename);

    const reportContent = `# Evaluation: ${company} - ${role}

**Date:** ${today}
**Archetype:** ${archetype}
**Score:** ${score}/5
**Legitimacy:** ${legitimacy}
**PDF:** pending
**Tool:** NVIDIA API (${modelName})

---

${evaluationText.replace(/---SCORE_SUMMARY---[\s\S]*?---END_SUMMARY---/, '').trim()}
`;

    writeFileSync(reportPath, reportContent, 'utf-8');
    console.log(`\nReport saved: reports/${filename}`);
    console.log(`Tracker row: | ${num} | ${today} | ${company} | ${role} | ${score} | Evaluada | ❌ | [${num}](reports/${filename}) |`);
  } catch (err) {
    console.warn(`Could not save report: ${err.message}`);
  }
}

console.log('\n' + '-'.repeat(66));
console.log(`Score: ${score}/5 | Archetype: ${archetype} | Legitimacy: ${legitimacy}`);
console.log('-'.repeat(66) + '\n');
